...
import sys
import numpy as np
import evaluation.instances2dict_with_polygons as cs

def parse_args():
    parser = argparse.ArgumentParser(description='Convert dataset')
    parser.add_argument(
        '--outdir', help="output dir for json files", 
        default="/data/cityscapes_dataset/cityscape/", type=str)
    parser.add_argument(
            ...
            for filename in sorted(files):
                if filename.endswith(ends_in % data_set.split('_')[0]):
                    ...
                    image['seg_file_name'] = filename[:-len(
                        ends_in % data_set.split('_')[0])] + '%s_instanceTrainIds.png' % data_set.split('_')[0]
                    images.append(image)
